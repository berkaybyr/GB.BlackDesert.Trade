...
// Type: GB.BlackDesert.Trade.Web.Lib.Manager.WaitSellItem
...
namespace BlackDesert.TradeMarket.Lib.Manager
{
    public class WaitSellItem
    {
        ...
        {
            lock (WaitSellItem._locker)
            {
                if (WaitSellItem._flag)
                    return;
                WaitSellItem._flag = true;
            }
            try
            {
                WaitSellItem.waitSellItem();
                ServerLogManager.serverLogWrite(ServerLogType.eRunTimer, string.Format("[{0}]waitSellItem", state));
            }
            catch (Exception ex)
            {
                LogUtil.WriteLog(string.Format("[{0}]waitSellItem Exception={1}", state, (object)ex.ToString()), "ERROR");
            }
            lock (WaitSellItem._locker)
                WaitSellItem._flag = false;
        }
        ...
        private static void waitSellItem()
        {
            ...
            else
            {
                ...
                commonDbResult.list.ForEach((Action<uspListWaitBiddingSell_Result>)(waitItem =>
                {
                    ...
                    if (model.waitNo <= 0L)
                    {
                        LogUtil.WriteLog(string.Format("[Item Error]waitSellItem({0}) waitNo invaild{1}", (object)model.toString(), (object)model.waitNo), "WARN");
                    }
                    else
                    {
                        ...
                        if (num3 != 0L)
                        {
                            LogUtil.WriteLog(string.Format("[Item Error]waitSellItem({0}) sellPrice Check Fail unitPrice({1} divide Result {2}", (object)model.toString(), (object)unitPrice, (object)num3), "WARN");
                        }
                        else
                        {
                            ...
                            if (!info1.isValid())
                            {
                                LogUtil.WriteLog(string.Format("[Item Error]waitSellItem({0}) itemInfo({1}, {2}) Not Exist ItemInfo", (object)model.toString(), (object)model.sellMainKey, (object)model.sellChooseKey), "WARN");
                            }
                            else
                            {
                                ...
                                {
                                    LogUtil.WriteLog(string.Format("[Item Error]waitSellItem({0}) sellCount({1}) Process Count Over", (object)model.toString(), (object)model.sellCount), "WARN");
                                }
                                else
                                {
                                    ...
                                    if (info1._enchantMaterialKey != 0)
                                    {
                                        ...
                                        if (!info1.isValid())
                                        {
                                            LogUtil.WriteLog(string.Format("[Item Error]waitSellItem({0}) itemInfo({1}, 0) Not Exist Material ItemInfo", (object)model.toString(), (object)info1._enchantMaterialKey), "WARN");
                                            return;
                                        }
                                    }
                                    ...
                                    {
                                        LogUtil.WriteLog(string.Format("[Item Error]waitSellItem({0}) getEnchantMaxGroup({1}, {2}) Group : Out Of Range", (object)model.toString(), (object)model.sellMainKey, (object)info1._enchantGroup), "WARN");
                                    }
                                    else
                                    {
                                        if (model.sellSubKey != model.sellChooseKey)
                                        {
                                            ...
                                            if (!info2.isValid())
                                            {
                                                LogUtil.WriteLog(string.Format("[Item Error]waitSellItem({0}) baseitemInfo({1}, {2}) Not Exist ItemInfo", (object)model.toString(), (object)model.sellMainKey, (object)model.sellSubKey), "WARN");
                                                return;
                                            }
                                            ...
                                            {
                                                LogUtil.WriteLog(string.Format("[Item Error]waitSellItem({0}) getEnchantBaseMaxGroup({1}, {2}) Group : Out Of Range", (object)model.toString(), (object)model.sellMainKey, (object)info2._enchantGroup), "WARN");
                                                return;
                                            }
                                        }
                                        ...
                                        using (SA_BETA_TRADEDB_0002 SA_BETA_TRADEDB_0002 = new SA_BETA_TRADEDB_0002())
                                        {
                                            ...
                                            while (0L < sellCount)
                                            {
                                                try
                                                {
                                                    ...
                                                    if (int32_1 == 0)
                                                    {
                                                        if (StopReasonType.eStopReasonType_Empty != int32_2)
                                                        {
                                                            if (StopReasonType.eStopReasonType_Fail == int32_2)
                                                            {
                                                                if (5 == num7)
                                                                {
                                                                    LogUtil.WriteLog(string.Format("[DB Error]waitSellItem - uspSellBiddingToWorldMarket({0}/{1}) Fail BuyBidding", (object)model.toString(), (object)sellCount), "WARN");
                                                                    TradeModule.updateWaitFail(model, info1._weight, customTime);
                                                                    return;
                                                                }
                                                                ++num7;
                                                            ...
                                                            {
                                                                LogUtil.WriteLog(string.Format("[DB Error]waitSellItem - uspSellBiddingToWorldMarket({0}/{1}) StopReasonType({2}) is Invalid", (object)model.toString(), (object)sellCount, (object)(int)int32_2), "WARN");
                                                                TradeModule.updateWaitFail(model, info1._weight, customTime);
                                                                return;
                                                            }
                                                        }
                                                    ...
                                                    {
                                                        LogUtil.WriteLog(string.Format("[DB Error]waitSellItem - uspSellBiddingToWorldMarket({0}/{1}), rv({2})", (object)model.toString(), (object)sellCount, (object)int32_1), "WARN");
                                                        TradeModule.updateWaitFail(model, info1._weight, customTime);
                                                        return;
                                                    }
                                                }
                                                ...
                                                {
                                                    LogUtil.WriteLog(string.Format("[DB Exception]waitSellItem() - uspSellBiddingToWorldMarket({0}/{1}) Exception : {2}", (object)model.toString(), (object)sellCount, (object)ex.ToString()), "ERROR");
                                                    TradeModule.updateWaitFail(model, info1._weight, customTime);
                                                    return;
                                                }
                                            }
                                            ...
                                            else
                                            {
                                                try
                                                {
                                                    ...
                                                    {
                                                        LogUtil.WriteLog(string.Format("[DB Error]waitSellItem - uspSellToWorldMarket({0}/{1}), rv({2})", (object)model.toString(), (object)sellCount, (object)int32), "WARN");
                                                        TradeModule.updateWaitFail(model, info1._weight, customTime);
                                                    }
                                                }
                                                catch (Exception ex)
                                                {
                                                    LogUtil.WriteLog(string.Format("[DB Exception]waitSellItem() - uspSellToWorldMarket({0}/{1}) Exception : {2}", (object)model.toString(), (object)sellCount, (object)ex.ToString()), "ERROR");
                                                    TradeModule.updateWaitFail(model, info1._weight, customTime);
                                                }
                                            }
                                        }
