...
namespace BlackDesert.TradeMarket.Lib.Crypt
{
    ...
    {
        public string Encrypt(string strMsg, Enum enumType)
        {
            switch (enumType)
            {
                case Enum.Rc2:
                    ...
                    byte[] bytes2 = Encoding.UTF8.GetBytes(CON_RC2_KEY);
                    encryptor1.IV = Encoding.UTF8.GetBytes(CON_RC2_IV);
                ...
                case Enum.Des:
                    ...
                    byte[] bytes4 = Encoding.UTF8.GetBytes(CON_DES_KEY);
                    encryptor2.IV = Encoding.UTF8.GetBytes(CON_DES_IV);
                ...
                case Enum.TripleDes:
                    ...
                    byte[] bytes6 = Encoding.ASCII.GetBytes(CON_TRIPLEDES_KEY);
                    encryptor3.IV = Encoding.ASCII.GetBytes(CON_TRIPLEDES_IV);
                    return Convert.ToBase64String(encryptor3.Encrypt(bytes5, bytes6));
                case Enum.MD5:
                    byte[] hash1 = MD5.Create().ComputeHash(Encoding.Default.GetBytes(strMsg + CON_MD5_KEY + strMsg.Length.ToString()));
                    StringBuilder stringBuilder1 = new StringBuilder();
                    for (int index = 0; index < hash1.Length; ++index)
                        stringBuilder1.Append(hash1[index].ToString("x2"));
                    return stringBuilder1.ToString();
                case Enum.HEX:
                    byte[] bytes7 = Encoding.UTF8.GetBytes(CON_HEX_KEY);
                    byte[] bytes8 = Encoding.UTF8.GetBytes(CON_HEX_IV);
                    ...
                    CryptoStream cryptoStream = new CryptoStream(memoryStream, encryptor4, CryptoStreamMode.Write);
                ...
                case Enum.AES:
                    ...
                    string conRijndaePw = CON_RIJNDAE_PW;
                    string conRijndaeIv = CON_RIJNDAE_IV;
                    string conRijndaeSalt = CON_RIJNDAE_SALT;
                    int KeySize = 256;
                ...
                case Enum.SHA256:
                    using (SHA256Managed shA256Managed = new SHA256Managed())
                    {
                        ...
                        foreach (byte num in hash2)
                            stringBuilder2.AppendFormat("{0:x2}", num);
        ...
        public string Decrypt(string strEn, Enum enumType)
        {
            switch (enumType)
            {
                case Enum.Rc2:
                    Decryptor decryptor1 = new Decryptor(EncryptionAlgorithm.Rc2);
                    decryptor1.IV = Encoding.UTF8.GetBytes(CON_RC2_IV);
                    byte[] bytes1 = Encoding.UTF8.GetBytes(CON_RC2_KEY);
                    byte[] bytes2;
                    try
                    {
                ...
                case Enum.Des:
                    ...
                    decryptor2.IV = Encoding.UTF8.GetBytes(CON_DES_IV);
                    byte[] bytes3 = Encoding.UTF8.GetBytes(CON_DES_KEY);
                    byte[] bytes4;
                    try
                    {
                ...
                case Enum.TripleDes:
                    ...
                    decryptor3.IV = Encoding.UTF8.GetBytes(CON_TRIPLEDES_IV);
                    byte[] bytes5 = Encoding.UTF8.GetBytes(CON_TRIPLEDES_KEY);
                    byte[] bytes6;
                    try
                    {
                ...
                case Enum.HEX:
                    ...
                    CryptoStream cryptoStream = new CryptoStream(new MemoryStream(buffer1), decryptor4, CryptoStreamMode.Read);
                ...
                case Enum.RSA:
                    ...
                    try
                    {
                        ...
                        {
                            Modulus = Convert.FromBase64String(CON_RSA_N_KEY),
                            Exponent = Convert.FromBase64String(CON_RSA_E),
                            P = Convert.FromBase64String(CON_RSA_P),
                            Q = Convert.FromBase64String(CON_RSA_Q),
                            DP = Convert.FromBase64String(CON_RSA_DP),
                            DQ = Convert.FromBase64String(CON_RSA_DQ),
                            InverseQ = Convert.FromBase64String(CON_RSA_INVERSEQ),
                            D = Convert.FromBase64String(CON_RSA_D_KEY)
                        });
                        byte[] rgb = Convert.FromBase64String(strEn);
                        return ByteToString2(cryptoServiceProvider2.Decrypt(rgb, false));
                    }
                    catch
                    {
                        return "Error";
        ...
        public static bool blnMD5Hash(string strInput, string strHash) => StringComparer.OrdinalIgnoreCase.Compare(new SecurityLib().Encrypt(strInput, Enum.MD5), strHash) == 0;
