...
namespace BlackDesert.TradeMarket.Application.Manager
{
    public class UpdateSystemCount
    {
        ...
        public static void Excute(object state)
        {
            ...
            {
                LogUtil.WriteLog(string.Format("[{0}]UpdateSystemCount Exception={1}", state, ex.ToString()), "ERROR");
            }
            lock (_locker)
                _flag = false;
        }
                ...
                foreach (int num1 in TickCountManager.This().getMainGroupListXXX())
                {
                    if (num1 == WorldMarketOptionManager.This().getCashGroupNo())
                    {
                        ...
                        using (SA_BETA_TRADEDB_0002 SA_BETA_TRADEDB_0002 = new SA_BETA_TRADEDB_0002())
                        {
                            try
                            {
                                marketMainGroupList.list = ((IEnumerable<uspListWorldMarketByMainGroup_Result>)SA_BETA_TRADEDB_0002.uspListWorldMarketByMainGroup(new int?(num1), symNo, rv)).ToList();
                            }
                            catch (Exception ex)
                            {
                                LogUtil.WriteLog(string.Format("uspListWorldMarketByMainGroup Exception : {0}", ex.ToString()), "ERROR");
                                break;
                            }
                        }
                        foreach (uspListWorldMarketByMainGroup_Result byMainGroupResult in marketMainGroupList.list)
                        {
                            ...
                            {
                                int maxValue = (int)(((long)int.MaxValue >= byMainGroupResult.C_count ? (int)byMainGroupResult.C_count : (double)int.MaxValue) * (systemStockPercent / 1000000.0));
                                if (maxValue == 0)
                                    maxValue = 1;
