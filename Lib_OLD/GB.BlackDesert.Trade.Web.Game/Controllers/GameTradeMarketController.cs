...
// Type: GB.BlackDesert.Trade.Web.Game.Controllers.GameTradeMarketController
...
using GB.BlackDesert.Trade.Web.Game.Models;
...
namespace GB.BlackDesert.Trade.Web.Game.Controllers
{
    public class GameTradeMarketController : BaseController
    {
        ...
        public JsonResult SetMarketItem(GameTradeMarketItmeAuthModel model)
        {
            ...
            try
            {
                using (TradeGameDB tradeGameDb = new TradeGameDB(TradeModule.MakeGameDbName(this.Request.Url.Host, userInfo._worldNo)))
                {
        ...
        public JsonResult WithdrawItem(GameTradeMarketItmeAuthModel model)
        {
            ...
            string name = TradeModule.MakeGameDbName(this.Request.Url.Host, userInfo._worldNo);
            commonResult2.resultCode = WorldMarketBase.checkValidCount<int, long>(1, model.count);
            if (commonResult2.resultCode != 0)
            {
        ...
        public JsonResult CreateMyWallet(GameTradeMarketUserAuthModel authModel)
        {
            ...
            if (ConstantMgr._serviceProject.Equals("BDO"))
            {
                ...
                dateTime1 = TradeModule.GetValuePackageExpiration(userInfo, this.Request.Url.Host);
                dateTime2 = TradeModule.GetChargeBuffExpiration(userInfo, this.Request.Url.Host, (byte)0);
                num1 = TradeModule.getFamilyPoint(authModel.userNo, this.Request.Url.Host, userInfo._worldNo);
                num2 = TradeModule.getMaidCount(authModel.userNo, this.Request.Url.Host, userInfo._worldNo);
                flag = TradeModule.isGameMaster(authModel.userNo);
                worldMarketResultList = TradeModule.getFamilySkillList(authModel.userNo, this.Request.Url.Host, userInfo._worldNo);
                num3 = TradeModule.getRingBuffList(authModel.userNo, this.Request.Url.Host, userInfo._worldNo);
            }
        ...
        public JsonResult BuyItem(GameTradeMarketBuyAuth buyAuthmodel)
        {
            ...
            {
                commonResult2 = TradeModule.CheckTicketNo(userInfo._userNo, this.Request.Url.Host, userInfo._worldNo, (byte)0, buyAuthmodel.ticketNo);
                if (commonResult2.resultCode != 0)
                {
        ...
        public JsonResult SellItem(GameTradeMarketSellAuth sellAuthModel)
        {
            ...
            if (!userInfo._isWebAccess)
            {
                commonResult2 = TradeModule.CheckTicketNo(userInfo._userNo, this.Request.Url.Host, userInfo._worldNo, (byte)1, sellAuthModel.ticketNo);
                if (commonResult2.resultCode != 0)
                {
